...
Changes from 2.1.0: updated storage requirements to central {agent_root}/ layout; added journal path requirement; added interfaces section; added spec file reference table; clarified visibility and journal type requirements.
...
```
{agent_root}/data/{skill-name}/config.json    — skill configuration
{agent_root}/data/{skill-name}/*.jsonl        — append-only logs
{agent_root}/journals/{skill-name}/YYYY-MM-DD/{run_id}.json  — journal files
```

For LadybugDB skills only:
```
{agent_root}/db/{skill-name}/{skill-name}.lbug
```
...
```
{agent_root}/data/ocas-{skill}/
  config.json
  {primary_log}.jsonl
  decisions.jsonl
{agent_root}/journals/ocas-{skill}/
...
Journal path: `{agent_root}/journals/ocas-{skill}/YYYY-MM-DD/{run_id}.json`
...
**Storage Layout** — data and journal paths under `{agent_root}/`.
...
Be exact about: naming, file paths (use `{agent_root}/` root), metadata fields, command syntax, schemas, validation checks, routing language, journal types.
...
- Storage paths use `{agent_root}/` root
...
### Storage Layout
- Data path: `{agent_root}/data/ocas-{skill}/`
- Journal path: `{agent_root}/journals/ocas-{skill}/`
- DB path (if applicable): `{agent_root}/db/ocas-{skill}/`
