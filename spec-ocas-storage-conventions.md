...
Changes from 1.0: replaced workspace dot-folder convention with centralized storage under {agent_root}/; defined separate roots for data, journals, and databases; added LadybugDB database convention; added intake directory convention; clarified cross-skill access rules; updated initialization and validation sections.
...
All persistent OCAS data lives under a single central root: `{agent_root}/`.
...
No skill writes outside `{agent_root}/`. No skill writes inside the skill package directory. No skill writes into another skill's data or journal directory.
...
```
{agent_root}/data/{skill-name}/intake/
...
- Data root exists at `{agent_root}/data/{skill-name}/`
- Journal root exists at `{agent_root}/journals/{skill-name}/`
