...
```
{agent_root}/data/{skill-name}/   — state, config, JSONL logs
{agent_root}/journals/{skill-name}/YYYY-MM-DD/{run_id}.json  — journal files
```

LadybugDB skills only:
```
{agent_root}/db/{skill-name}/     — LadybugDB database files
```

Config file location: `{agent_root}/data/{skill-name}/config.json`
...
Journal file location: `{agent_root}/journals/{skill-name}/YYYY-MM-DD/{run_id}.json`
...
**Storage Layout** — the skill's data and journal paths under `{agent_root}/`.
...
Skills that are commonly invoked as part of multi-step cross-skill workflows should ship bundled plans. Plans are stored at `references/plans/` in the skill package and copied to `{agent_root}/data/ocas-mentor/plans/` during Mentor initialization.
...
3. Add plan copying to the skill's `init` command: copy `references/plans/*.plan.md` to `{agent_root}/data/ocas-mentor/plans/`, skipping files already present.
