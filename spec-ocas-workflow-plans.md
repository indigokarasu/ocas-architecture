...
- Plans are Markdown files with YAML frontmatter, stored at `{agent_root}/data/ocas-mentor/plans/`.
...
- Plans are discovered by scanning `{agent_root}/data/ocas-mentor/plans/` for `*.plan.md` files.
...
- Every plan run is tracked in `{agent_root}/data/ocas-mentor/plan-runs/{plan_run_id}/`.
...
- `mentor.plan.list` -- list all plans in `{agent_root}/data/ocas-mentor/plans/` with plan_id, version, and description
...
2. Place it in `{agent_root}/data/ocas-mentor/plans/` for user-created plans, or in a skill's `references/plans/` for bundled plans.
...
The following skills are expected to ship bundled plans. Plans are copied to `{agent_root}/data/ocas-mentor/plans/` during `mentor.init`. Existing files are never overwritten (user modifications are preserved).
...
```
Copy all *.plan.md files from {skill}/references/plans/ to {agent_root}/data/ocas-mentor/plans/
