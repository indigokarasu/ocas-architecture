...
This document defines the contracts for all inter-skill communication in the OCAS ecosystem. Skills communicate through shared filesystem paths under `{agent_root}/`, not through direct calls or shared memory.
...
### Path
```
{agent_root}/db/ocas-elephas/intake/{signal_id}.signal.json
```
...
### Consumption
Mentor scans `{agent_root}/journals/` recursively during `mentor.heartbeat.light` and `mentor.heartbeat.deep`. It tracks which `run_id`s have already been ingested via its own ingestion log at `{agent_root}/data/ocas-mentor/ingestion_log.jsonl`.
...
### Path
```
{agent_root}/data/ocas-praxis/intake/{signal_id}.json
```
...
### Path
```
{agent_root}/data/ocas-forge/intake/{proposal_id}.json
```
...
### Path
```
{agent_root}/data/ocas-forge/intake/{decision_id}.json
```
...
### Path
```
{agent_root}/data/ocas-vesper/intake/{proposal_id}.json
```
...
### Path
```
{agent_root}/data/ocas-corvus/intake/{thread_id}.json
```
...
### Path
```
{agent_root}/db/ocas-elephas/intake/{candidate_id}.signal.json
```
...
```
{agent_root}/data/ocas-rally/reports/{YYYY-MM-DD}-daily.json
```
...
### Path
```
{agent_root}/data/ocas-fellow/intake/{experiment_id}.json
```
...
### Path
```
{agent_root}/data/ocas-mentor/intake/{cycle_id}.json
```
...
```
{agent_root}/data/ocas-thread/active_context.json
```
